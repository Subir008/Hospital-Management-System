...
public class Update_Password extends HttpServlet {
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// First check the id and password is present there in db or not if present go
		// to if block otherwise else
		if (flag) {
			// If the old password is present than check the new given password is equals to
			// the confirm password or not if they matches then go to if block and update
			// the data otherwise through error
